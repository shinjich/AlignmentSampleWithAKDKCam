...
LPBYTE g_pPixel[4] = { NULL, };				// ビットマップの中身の先頭 (ピクセル情報)
int g_bSendToAKDKCamLib = 0;
...
uint32_t KinectProc()
{
	...
	if ( hr == K4A_WAIT_RESULT_SUCCEEDED )
	{
		...
		if ( hDepthImage )
		{
			...
			if ( pDepth )
			{
				...
				for( uint32_t u = 0; u < uImageSize; u++ )
				{
					...
					g_pPixel[0][u * 3 + 0] = (BYTE) w;
					g_pPixel[0][u * 3 + 1] = (BYTE) w;
					g_pPixel[0][u * 3 + 2] = (BYTE) w;
				}
				...
				if ( rTransColorToDepth == K4A_RESULT_SUCCEEDED )
				{
					...
					if ( pTrans )
					{
						...
						for( uint32_t u = 0; u < uImageSize; u++ )
						{
							...
							if ( w )
							{
								g_pPixel[2][u * 3 + 0] = (pTrans[u] >> 0) & 0xFF;
								g_pPixel[2][u * 3 + 1] = (pTrans[u] >> 8) & 0xFF;
								g_pPixel[2][u * 3 + 2] = (pTrans[u] >> 16) & 0xFF;
							}
							else
							{
								g_pPixel[2][u * 3 + 0] = 0;
		...
		if ( hColorImage )
		{
			...
			{
				// イメージサイズを取得する
				uImageSize = (uint32_t) k4a_image_get_size( hColorImage ) / sizeof(uint32_t);
				for( uint32_t u = 0; u < uImageSize; u++ )
				{
					g_pPixel[1][u * 3 + 0] = (pColor[u] >> 0) & 0xFF;
					g_pPixel[1][u * 3 + 1] = (pColor[u] >> 8) & 0xFF;
					g_pPixel[1][u * 3 + 2] = (pColor[u] >> 16) & 0xFF;
				}
				...
				if ( rTransDepthToColor == K4A_RESULT_SUCCEEDED )
				{
					...
					if ( pTrans )
					{
						...
						for( uint32_t u = 0; u < uImageSize; u++ )
						{
							...
							if ( w )
							{
								g_pPixel[3][u * 3 + 0] = (pColor[u] >> 0) & 0xFF;
								g_pPixel[3][u * 3 + 1] = (pColor[u] >> 8) & 0xFF;
								g_pPixel[3][u * 3 + 2] = (pColor[u] >> 16) & 0xFF;
							}
							else
							{
								g_pPixel[3][u * 3 + 0] = 0;
...
HRESULT InitApp( HINSTANCE hInst, int nCmdShow )
{
	...
	g_biBMP[0].bmiHeader.biBitCount = 24;
	...
	g_hBMP[0] = CreateDIBSection( NULL, &g_biBMP[0], DIB_RGB_COLORS, (LPVOID*) (&g_pPixel[0]), NULL, 0 );

	ZeroMemory( &g_biBMP[1], sizeof(g_biBMP[1]) );
	g_biBMP[1].bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	g_biBMP[1].bmiHeader.biBitCount = 24;
	...
	g_hBMP[1] = CreateDIBSection( NULL, &g_biBMP[1], DIB_RGB_COLORS, (LPVOID*) (&g_pPixel[1]), NULL, 0 );

	ZeroMemory( &g_biBMP[2], sizeof(g_biBMP[2]) );
	g_biBMP[2].bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	g_biBMP[2].bmiHeader.biBitCount = 24;
	...
	g_hBMP[2] = CreateDIBSection( NULL, &g_biBMP[2], DIB_RGB_COLORS, (LPVOID*) (&g_pPixel[2]), NULL, 0 );

	ZeroMemory( &g_biBMP[3], sizeof(g_biBMP[3]) );
	g_biBMP[3].bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	g_biBMP[3].bmiHeader.biBitCount = 24;
	...
	g_hBMP[3] = CreateDIBSection( NULL, &g_biBMP[3], DIB_RGB_COLORS, (LPVOID*) (&g_pPixel[3]), NULL, 0 );
...
int WINAPI WinMain( HINSTANCE hInst, HINSTANCE, LPSTR, int nCmdShow )
{
	...
	AKDKCamCreate( g_hAzureKinect, COLOR_RESOLUTION_WIDTH, COLOR_RESOLUTION_HEIGHT );
	...
	while( GetMessage( &msg, NULL, 0, 0 ) )
	{
		...
		if ( KinectProc() )
		{
			if ( g_bSendToAKDKCamLib >= 0 )
				AKDKCamSetBGR( g_pPixel[g_bSendToAKDKCamLib], g_biBMP[g_bSendToAKDKCamLib].bmiHeader.biWidth, g_biBMP[g_bSendToAKDKCamLib].bmiHeader.biHeight );
